...
// result interface defines exactly what we report back
interface GhostCheckResult {
  ghosts: string[];      // The unused packages
  totalDeps: number;     // Total production deps found
  usedDeps: number;      // How many were actually used
  scannedFiles: number;  // Metric to prove we actually worked
}

export async function checkGhostImports(
  projectRoot: string,
  ignoreList: string[],
  tsConfigPath: string,
  sourceGlobs: string[]
  ...
  for (const sourceFile of sourceFiles) {
    try {
      ...
      sourceFile.getExportDeclarations().forEach(exp => {
        ...
        if (val) usedPackages.add(extractPackageName(val));
      });
  ...
  const ghosts = dependencies.filter(dep => {
    if (usedPackages.has(dep)) return false; // Used? Keep it.
    if (ignoreList.includes(dep)) return false; // Whitelisted? Keep it.
    if (dep.startsWith('@types/')) return false; // Type definition? Keep it.
